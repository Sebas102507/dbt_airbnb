-- This model cleans and standardizes the G02 Census data.
-- It dynamically renames all columns to lowercase and cleans the LGA code.
...
SELECT
    ...
    {% for col in adapter.get_columns_in_relation(source('bronze', 'census_g02_nsw_lga_bronze')) %}
        ...
        -- If the column is the LGA code, remove the 'LGA' prefix and cast to integer
        {% if col.name == 'LGA_CODE_2016' %}
            CAST(REPLACE("{{ col.name }}", 'LGA', '') AS VARCHAR) AS lga_code

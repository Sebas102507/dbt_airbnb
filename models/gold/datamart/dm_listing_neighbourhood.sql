...
enriched_listings AS (
    SELECT
        ...
        scd_host.host_id -- Added the natural host_id key here
    FROM fct_listings
...
monthly_agg AS (
    SELECT
        ...
        COUNT(DISTINCT host_id) AS distinct_hosts, -- Corrected to count host_id
        COUNT(DISTINCT CASE WHEN is_superhost THEN host_id END) AS superhost_count, -- Corrected to count host_id
